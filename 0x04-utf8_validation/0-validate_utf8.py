...
def validUTF8(data):
    ...
    return True
    """
    # Initialize nmber of expected continuation bytes
    n_bytes = 0

    for num in data:
        if n_bytes == 0:
            # Determine the number of bytehe character
            if num >> 5 == 0b110:
                n_bytes = 1
            elif num >> 4 == 0b1110:
                n_bytes = 2
                ...
                # If first bit is 1, but it doesn't match valid pattern
                return False
        else:
            # Check contiation bte
            if num >> 6 != 0b10:
                return False
            n_bytes -= 1

    # If n_bytes is nt 0, it means w expecting more continuation bytes
    return n_bytes == 0
